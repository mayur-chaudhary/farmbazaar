...
const NavBarDeliveryPartner = () => {
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    const userData = JSON.parse(sessionStorage.getItem('userData'));
    if (!userData || !userData.isLoggedIn || userData.role !== 'DELIVERY_PARTNER') {
      // Redirect to login if not logged in or not a DELIVERY_PARTNER
      navigate('/signin');
    }
  }, [navigate]);
    ...
    <>
      <nav style={{backgroundColor: 'transparent', marginBottom: '20px'}} className="navbar navbar-expand-lg bg-light">
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}} className="container-fluid">
          <div>
            <Link to="/delivery-partner-dashboard" onClick={handleLogoClick} style={{textDecoration: 'none', color: '#12372A', display: 'flex', alignItems: 'center'}} className="navbar-brand">
...
export default NavBarDeliveryPartner;
